...
function CardValueComparison({ newValue, oldValue, description }: { newValue: number, oldValue: number, description:string }) {

    const result = PercentageDifference(newValue, oldValue);
    ...
    return (
        <div className="flex flex-row items-center justify-center gap-2">
            ...
            <span className={`text-preset-6 ${result > 0 ? "text-green-500" : "text-red-500"} `}>{result.toString().replace("-", "")}
                {description}
            </span>
        </div>
    );

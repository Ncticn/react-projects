...
export default function CardBodyMassIndexCalculator() {
  ...
  useEffect(() => {
    const bmi = bmiCalculateImperial(
      Number(heightFt),
      Number(heightIn),
      Number(weightSt),
      Number(weightLbs),
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl bg-white px-6 py-6 drop-shadow-[16px_32px_56px_rgba(143,174,207,25%)] md:px-8 md:py-8"
    >
      <div className="space-y-6 md:space-y-8">
        ...
        <form onSubmit={(e) => e.preventDefault()}>
          <div className="flex flex-col items-stretch justify-center gap-6 md:gap-8">
            ...
            <div className="grid grid-cols-1 items-end justify-center gap-4 md:grid-cols-2 md:gap-6">
            ...
            <BarInformation bmi={Number(result.bmi)}>
              {result.message} Your ideal weight is between{" "}
              <b>

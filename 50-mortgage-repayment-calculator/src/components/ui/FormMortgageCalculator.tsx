...
export default function FormMortgageCalculator({ calculatedResult }: FormMortgageCalculatorProps) {
    ...
    const handleInputMortgageAmount = (value: string) => {
        ...
        if (clearValue === "") {
            setInputMortgageAmount({ value: clearValue, message: "This field is required" });
        } else {
            const formatValue = utilNumberToFormatCurrency(Number(clearValue), "en-US");
    ...
    const handleInputMortgageTerm = (value: string) => {
        ...
        if (clearValue === "") setInputMortgageTerm({ value: "", message: "This field is required" });
    ...
    const handleFormSubmit = () => {
        if (inputMortgageAmount.value === "" || inputMortgageTerm.value === "" || inputInterestRate.value === "" || selected === "") {
            if (inputMortgageAmount.value === "") setInputMortgageAmount({ value: "", message: "This field is required" })
            if (inputMortgageTerm.value === "") setInputMortgageTerm({ value: "", message: "This field is required" })
            if (inputInterestRate.value === "") setInputInterestRate({ value: "", message: "This field is required" })
            if (selected === "") setSelected(() => "error");
        } else {
            const numberMortgageAmount = Number(inputMortgageAmount.value.replaceAll(",", ""));
            ...
            if (selected === "repayment") {
                const result = utilCalculateMortgage("repayment", numberMortgageAmount, numberMortgageTerm, numberMortgageRate);
                calculatedResult(result, result * (numberMortgageTerm * 12));
            } else if (selected === "interestOnly") {
                const result = utilCalculateMortgage("interestOnly", numberMortgageAmount, numberMortgageTerm, numberMortgageRate);
    ...
    return (
        <form onSubmit={(e) => {
            ...
            className="px-6 py-6 md:px-10 md:py-10 flex flex-col items-stretch justify-start gap-6 md:gap-10">
            <div className="form-header flex flex-col md:flex-row items-start justify-start gap-2 md:items-center md:justify-between md:gap-0">
            ...
            <div className="form-fields flex flex-col items-stretch justify-start gap-6">
                ...
                <div className="flex flex-col items-stretch justify-start gap-3">
                    ...
                    {
                        selected === "error" &&
                        <span className="text-preset-5 text-red">This field is required</span>
                    }
                </div>
            </div>

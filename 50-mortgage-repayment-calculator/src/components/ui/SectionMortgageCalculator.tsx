...
export default function SectionMortgageCalculator() {
  ...
  const handleFormSubmit = (repayment?: number, totalRepay?: number) => {
    if (repayment) {
      setRepayment(
        utilNumberToFormatCurrency(Number(repayment?.toFixed(2)), "en-US"),
      );
      if (totalRepay) {
        setTotalRepayment(
          utilNumberToFormatCurrency(Number(totalRepay?.toFixed(2)), "en-US"),
        );
      } else {
        setTotalRepayment("");
      }
  ...
  return (
    <section
      aria-label="Section Mortgage Calculator"
      className="relative overflow-hidden bg-white drop-shadow-[0_32px_64px_rgba(19,45,65,10%)] md:rounded-3xl md:rounded-bl-[80px]! lg:rounded-3xl!"
    >
      <div className="grid grid-cols-1 md:grid-rows-2 lg:grid-cols-2 lg:grid-rows-1">
        <FormMortgageCalculator calculatedResult={handleFormSubmit} />

        {repayment ? (
          <div className="col-span-1 bg-slate-900 px-6 py-8 *:h-full md:rounded-bl-[80px] md:px-10 md:py-10">
            <div className="flex flex-col items-stretch justify-start gap-6 md:gap-10">
              ...
              <CardResult
                monthlyRepayments={repayment}
                totalRepay={totalRepayment}
              />
            </div>
          </div>
        ) : (
          <div className="bg-slate-900 px-6 py-8 *:h-full md:rounded-bl-[80px] md:px-10 md:py-10">
            <div className="flex flex-col items-center justify-center gap-4 text-center">
              <Image
                src={IllustrationEmpty}
                alt="Image Illustration Empty"
                loading="eager"
              />

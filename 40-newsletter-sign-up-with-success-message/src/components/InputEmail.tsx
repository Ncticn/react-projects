...
export default function InputEmail({ inputLabel = "Label", placeholder = "example@email.com", inputID, inputName = "input-user-email" }: InputEmailProps) {
    ...
    return (
        <div className="form-input-item flex flex-col gap-2 items-start justify-start">
            <div className="flex flex-row items-center justify-between w-full">
                <label htmlFor="input-user-email" className="text-preset-3 text-blue-800">{inputLabel}</label>
                <span className={`text-preset-3 text-red ${error && inputValue ? "opacity-100" : "opacity-0"} transition delay-75 ease-linear`}>Valid email required</span>
            </div>
